...
import { createChoferProfile, getChoferProfile } from "./actions";
...
type State = "loading" | "selection" | "waiting" | "active";

export default function OnboardingPage() {
  const { user, isLoaded } = useUser();
  const router = useRouter();
  const [state, setState] = useState<State>("loading");
  const [choferProfile, setChoferProfile] = useState<ChoferProfile | null>(null);
  ...
  // Cargar perfil y verificar estado
  useEffect(() => {
    if (!isLoaded) return;
    ...
    async function loadProfile() {
      ...
      if (result.chofer) {
        const db = result.chofer;
        setChoferProfile({
          idChofer: db.idChofer,
          nombre: db.nombre ?? "",
          telefono: db.telefono ?? "",
          estado: db.estado ?? "pendiente",
          idVendedor: db.idVendedor,
          idVehiculo: db.idVehiculo ?? null,
          vehiculo: db.vehiculo
            ? { patente: db.vehiculo.patente, tipo: db.vehiculo.tipo }
            : null,
        });

        // Obtener nombre del vendedor
        const vendorsResponse = await fetch("/api/vendors");
        const vendorsList = (await vendorsResponse.json()) as Vendor[];
        const vendor = vendorsList.find(
          (v) => v.id_vendedor === result.chofer!.idVendedor
        );
        if (vendor) {
          setVendorName(vendor.nombre);
        }

        if (result.chofer.idVehiculo) {
          setState("active");
        } else {
          setState("waiting");
        }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const result = await createChoferProfile({
      ...
      if (result.ok) {
        const profileResult = await getChoferProfile();
        if (profileResult.chofer) {
          const db = profileResult.chofer;
          setChoferProfile({
            idChofer: db.idChofer,
            nombre: db.nombre ?? "",
            telefono: db.telefono ?? "",
          ...
          const vendor = vendors.find(
            (v) => v.id_vendedor === profileResult.chofer!.idVendedor
          );
          if (vendor) {
            setVendorName(vendor.nombre);
          }
        ...
        <div className="max-w-md mx-auto text-center">
          <div className="bg-white rounded-lg shadow-lg p-8">
            ...
            <p className="text-xl text-gray-700 mb-2">
              Tu solicitud está siendo procesada por
            </p>
            <p
            ...
            <p className="text-gray-600 mb-6">
              Pronto se te asignará un vehículo para que puedas comenzar a
              trabajar.
            </p>
            <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded">
              <p className="text-sm text-gray-700">
                💡 Recibiras una notificación cuando tu vehículo esté listo.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-lg shadow-lg p-8 mb-6">
            <div className="mb-6">
              <h1 className="text-3xl font-bold text-gray-900 mb-1">
                {choferProfile.nombre}
              </h1>
              <p className="text-gray-600">Empresa: {vendorName}</p>
            </div>
